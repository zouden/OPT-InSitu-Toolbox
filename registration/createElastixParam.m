...
addOptional(p,paramUseDirectionCosines,'true',@(x) any(validatestring(x,{'true','false'})));
addOptional(p,paramTransform,'affine',@(x) any(validatestring(x,{'affine','similarity','rigid','tps','bspline'})));
addOptional(p,paramMetric,'corr',@(x) any(validatestring(x,{'corr','mm','sd','inv','ngc','pi','corr_rigidp','corr_bendp'})));
...
addOptional(p,paramResampleInter,'bspline',@(x) any(validatestring(x,{'linear','bspline1','bspline2','bspline3','nearest'})));
addOptional(p,paramInter,'linear',@(x) any(validatestring(x,{'linear','bspline1','bspline2','bspline3','nearest'})));
addOptional(p,paramGPU,0,@(x) isnumeric(x));
addOptional(p,paramOptimizer,'adaptive',@(x) any(validatestring(x,{'adaptive','regular'})));
